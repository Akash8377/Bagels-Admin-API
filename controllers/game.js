...
exports.register = (req, res) => {
  ...
  const gameName = req.body.game_name.toLowerCase();
  const sqlCheckQuery = `SELECT * FROM ${WEEK_TABLE} WHERE LOWER(game_name) = ?`;

  conn.query(sqlCheckQuery, [gameName], (err, result) => {
    if (err) {
    ...
    if (result && result.length) {
      return res.status(409).send({
        msg: "This Game already exists",
      });
    } else {
      const date_time = new Date();
      const sqlQuery = `INSERT INTO ${WEEK_TABLE} (game_name, created_at, updated_at) VALUES (?, ?, ?)`;
      const values = [
        req.body.game_name,
        date_time,
        date_time
      ...
      conn.query(sqlQuery, values, (err, result) => {
        if (err) {
          ...
          res.status(200).send({
            status: "success",
            msg: "Game registered successfully",
          });
        }
      });
    }
